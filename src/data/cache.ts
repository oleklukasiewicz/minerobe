...
import { OutfitPackageLink, WardrobePackage } from "./common";
...
export const wardrobeNormalized: Readable<WardrobePackage> = derived(
  wardrobe,
  ($wardrobe) => {
    const copy= Object.assign({}, $wardrobe);
    if ($wardrobe == null || $wardrobe.sets == null) return $wardrobe;
    copy.sets = $wardrobe.sets.map((item) => {
      if (item.isShared == false) return item;
      const link = new OutfitPackageLink(item.id, item.model);
      return link;
    }) as [];
    return copy;
  }
...
itemPackage.subscribe(async (data: OutfitPackage) => {
  if (
    data != null &&
    data.publisher != null &&
    data.publisher.id == get(currentUser).id
  ) {
    ...
    wardrobe.update((w) => {
      ...
      if (data.id != null) {
        if (data.type == PACKAGE_TYPE.OUTFIT_SET) {
          const index = w.sets.findIndex((x) => x.id == data.id);
          if (index != -1) w.sets[index] = data;
        } else {
          const index = w.outfits.findIndex((x) => x.id == data.id);
...
currentUser.subscribe(async (user) => {
  if (user) {
    //settings up account
    itemPackage.set(
      new OutfitPackage("default", "alex", [], PACKAGE_TYPE.OUTFIT, null)
    );
...
wardrobeNormalized.subscribe(async (data) => {
